...
using TextsN = Sitecore.Names.Texts;
...
namespace Sitecore.Support.Controllers
{
    ...
    public class SupportMediaController : Controller
    {
        ...
        private static bool ValidateDestination(string database, string destinationUrl, SitecoreViewModelResult result)
        {
            ...
            if (item == null)
            {
                errorItems.Add(new ErrorItem("destination", destinationUrl, ClientHost.Globalization.Translate(TextsN.NoRootItemFound)));
        ...
        private static bool ValidateFile(HttpPostedFileBase file, SitecoreViewModelResult result)
        {
            ...
            {
                errorItems.Add(new ErrorItem("size", fileSize.ToString(), string.Format(ClientHost.Globalization.Translate(TextsN.FileSizeError), Settings.Media.MaxSizeInDatabase)));
                Log.Error(string.Format(string.Format(TextsN.FileSizeError, Settings.Media.MaxSizeInDatabase)), result);
                isValid = false;
            }
        ...
        private JsonResult DoUpload(string database, string destinationUrl)
        {
            ...
            if (!string.IsNullOrEmpty(uploadPipelineArgs.ErrorText))
            {
                ...
                errorResult.Result.errorItems = new List<ErrorItem>
                {
                  new ErrorItem("uiUpload", string.Empty, TextsN.UploadPipelineError),
